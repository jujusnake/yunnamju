...
const Works = () => {
  ...
  const [detailItem, setDetailItem] = useState<WorkItem>();
  ...
  return (
    <>
        ...
        <main className="px-6 pb-6 text-typo-main md:p-12">
          ...
          <section>
            {workList.map((work) => (
              <WorksBlock
                item={work}
                key={`work-item-${currTab}-${work.title}`}
                onClick={() => {
      ...
      <div
        ...
        } w-dvw h-dvh bg-layer-back fixed top-0 left-0 transition-[transform,_opacity] duration-[400ms] ease-in p-6`}
      >
        <div className="flex flex-col h-full border border-border-1">
          <button onClick={() => setIsDetailOpen(false)} className="self-end p-3">
            <X />
          </button>
        </div>
      </div>
...
const WorksBlock = ({ item, onClick }: { item: WorkItem; onClick?: () => void }) => {
  return (
    <article
      className="group relative cursor-pointer grid grid-cols-1 pt-6 pb-4 border-b border-border-1 last:border-b-0 md:py-9 md:grid-cols-[repeat(2,_auto)] md:grid-rows-2 md:justify-between md:gap-x-3 md:hover:bg-layer-front transition-colors"
      onClick={onClick}
    >
      <div>
        <h1 className="font-semibold text-[16px] leading-[150%] mb-0.5 md:text-[20px] flex items-center gap-2">
          {item.title}
        </h1>
      ...
      <aside className="mt-3 md:absolute text-xs italic md:text-sm font-semibold md:bottom-1.5 md:right-2 md:invisible md:opacity-0 group-hover:visible group-hover:opacity-100 transition-[visibility,_opacity]">
        Click to read more →
      </aside>
    </article>
  );
